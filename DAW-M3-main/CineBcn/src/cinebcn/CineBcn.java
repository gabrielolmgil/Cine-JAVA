import cinebcn.exceptions.CapacidadSalaException;
import cinebcn.exceptions.TiposSalaExcpetion;
...
public class CineBcn {
        ...
        do {
            ...
            switch (opcion) {
                case 1:
                    try {
                    agregarSala();
                    } catch (CapacidadSalaException ex){
                        System.out.println(ex.getMessage());
                    }catch (TiposSalaExcpetion ex){
                        System.out.println(ex.getMessage());
                    }
                    break;
                case 2:
    ...
    private static void agregarSala() throws CapacidadSalaException, TiposSalaExcpetion {
        System.out.print("Ingrese el ID de la sala: ");
        int id = scanner.nextInt();
        System.out.print("Ingrese la capacidad de la sala: ");
        int capacidad = scanner.nextInt();
        if (capacidad > 200){
            throw new CapacidadSalaException("No puede haber una sala con mas de 200 butacas");
        }
        scanner.nextLine(); // Consumir el salto de línea
        System.out.print("Ingrese el tipo de sala (IMAX, 3D, Normal): ");
        String tipo = scanner.nextLine();
        if (tipo != "IMAX" && tipo != "3D" && tipo != "Normal") {
            throw new TiposSalaExcpetion ("El tipo de sala que has puesto no exsiste");
        }
